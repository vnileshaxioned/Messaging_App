    ...
    try {
        ...
        || $check_file)) {
            ...
            if ($email_exist) {
                ...
                $columns = array('name', 'email', 'phone_number', 'gender', 'password', 'profile_image');
                $insert_data = $userData->insertQuery('users', 'ssssss', $values, $columns);
                if ($insert_data) {
...
if (isset($_POST['send_message'])) {
    $user_id = $validate->validateInput($_POST['user_id']);
    $to_user = $validate->validateInput($_POST['to_user']);
    $message = $validate->validateInput($_POST['message']);
    $time = $validate->validateInput(date("h:i:s A"));
    $date = $validate->validateInput(date("d-m-Y"));

    try {
        if ($message != '') {
            $values = array($user_id, $to_user, $message, $time, $date);
            $columns = array('user_id', 'to_user', 'message', 'time', 'date');
            $insert_data = $userData->insertQuery('chats', 'issss', $values, $columns);
            if ($insert_data) {
                $userData->redirect("", "chat.php?id=$to_user");
            } else {
        ...
        $userData->redirect($e->getMessage(), "chat.php?id=$to_user");
    }
}
...
if (isset($_POST['send_group_message'])) {
    ...
    try {
        if ($message != '') {
            $values = array($user_id, $message, $time, $date);
            $columns = array('user_id', 'message', 'time', 'date');
            $insert_data = $userData->insertQuery('group_chats', 'isss', $values, $columns);
            if ($insert_data) {
                $userData->redirect("", "group_chat.php");
            } else {
                throw new Exception ("Message not inserted");
            }
        } else {
        ...
        $userData->redirect($e->getMessage(), "group_chat.php");
    }
}
